...
void pwr_state_check_handler(struct timer_list* timer)
{
	struct dvobj_priv *dvobj = from_timer(dvobj, timer, pwrctl_priv.pwr_state_check_timer);
	rtw_ps_cmd(dvobj->if1);
}
...
_func_enter_;
	...
	rtw_init_timer(&pwrctrlpriv->pwr_state_check_timer, adapter_to_dvobj(padapter), pwr_state_check_handler);
