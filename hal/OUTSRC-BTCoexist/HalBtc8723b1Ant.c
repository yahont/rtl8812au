...
{	
	...
	snprintf(cliBuf, BT_TMP_BUF_SIZE, "\r\n============[Antenna Detection info]  ============\n");
	CL_PRINTF(cliBuf);
	...
	{
		case 0:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(BT is not available)");
			break;
		case 1:  // 2-Ant bad-isolation
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(BT is available)");
			break;
		...
		case 3:  // 1-Ant 
			...
			break;
		case 4:  
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(Uncertainty result)");
			break;
		case 5:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(Pre-Scan fai)");
			break;	
		case 6:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(WiFi is Scanning)");
			break;	
		case 7:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(BT is not idle)");
			break;	
		case 8:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(Abort by WiFi Scanning)");
			break;		
		case 9:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(Antenna Init is not ready)");
			break;	
		case 10:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(BT is Inquiry or page)");
			break;	
		case 11:
			snprintf(cliBuf, BT_TMP_BUF_SIZE, "(BT is Disabled)");
			break;		
}
	CL_PRINTF(cliBuf);	
	...
	)
{
	...
	{	
		...
		if (PsdRep2 < 10)	
			snprintf(cliBuf, BT_TMP_BUF_SIZE, ", Value = %d.0%d dB, (%d) \n",  
			  PsdRep1, PsdRep2, pPsdScan->nPSDMaxValue);
		else
			snprintf(cliBuf, BT_TMP_BUF_SIZE, ", Value = %d.%d dB, (%d)\n",  
			  PsdRep1, PsdRep2, pPsdScan->nPSDMaxValue);
	...
	else
	{
	 ...
	 while(1)
	 {
		do
		{
			...
			{
				if (freq2 == 0)
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.000\n", freq1);
				else if (freq2 < 100)	
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.0%2d\n", freq1,freq2);
				else
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.%3d\n", freq1,freq2);
			}			
			else
			{
				if (freq2 == 0)
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.000", freq1);
				else if (freq2 < 100)	
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.0%2d", freq1,freq2);
				else
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%6d.%3d", freq1,freq2);
			}
		...
		do
		{
			...
			{
				if (PsdRep2 <10)
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%7d.0%d\n", PsdRep1,PsdRep2);
				else
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%7d.%d\n", PsdRep1,PsdRep2);
			}
			else
			{
				if (PsdRep2 <10)
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%7d.0%d", PsdRep1,PsdRep2);
				else
					snprintf(cliBuf, BT_TMP_BUF_SIZE, "%7d.%d", PsdRep1,PsdRep2);
			}
	...
	)
{
				...
				for (pPsdScan->nPSDGenCount=1; pPsdScan->nPSDGenCount<=3; pPsdScan->nPSDGenCount++)
	{
				...
				if (freq2 < 100)
		{
					...
					snprintf(pPsdScan->nAntDet_PeakFreq, BT_8723B_1ANT_ANTDET_BUF_LEN, "%d.0%d", freq1,freq2);
				}
				else
			{
					RT_TRACE(COMP_COEX, DBG_LOUD, ("xxxxxxxxxxxxxxxx AntennaDetect(), Max Value: Freq = %d.%d MHz",  freq1, freq2));
					snprintf(pPsdScan->nAntDet_PeakFreq, BT_8723B_1ANT_ANTDET_BUF_LEN, "%d.%d", freq1,freq2);
				}

				if (PsdRep2 < 10)	
				{
					RT_TRACE(COMP_COEX, DBG_LOUD, (", Value = %d.0%d dB\n",   PsdRep1, PsdRep2));
					snprintf(pPsdScan->nAntDet_PeakVal, BT_8723B_1ANT_ANTDET_BUF_LEN, "%d.0%d", PsdRep1,PsdRep2);
				}
				else
				{
					RT_TRACE(COMP_COEX, DBG_LOUD, (", Value = %d.%d dB\n",PsdRep1, PsdRep2));
					snprintf(pPsdScan->nAntDet_PeakVal, BT_8723B_1ANT_ANTDET_BUF_LEN, "%d.%d", PsdRep1,PsdRep2);
				}
