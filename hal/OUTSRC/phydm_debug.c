...
phydm_BB_Debug_Info(IN PDM_ODM_T pDM_Odm)
{
	...
	snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "\r\n %-35s \n", "BB Report Info");
	...
	*/
	...
	if(RX_HT == 2)
	{
		...
		if(RX_VHT_BW==0)
		{
			snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "     BW=20M");
			DCMD_Printf(BbDbgBuf);
		}	
		else if(RX_VHT_BW==1)
		{
			snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "     BW=40M");
			DCMD_Printf(BbDbgBuf);
		}
		else
		{
			snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "     BW=80M");
			DCMD_Printf(BbDbgBuf);
		}
		RX_BW = RX_VHT_BW;
	}
	...
	/*
	if(RX_HT == 2){
		snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "  BW:%s", BW_table[RX_VHT_BW]);
		RX_BW = RX_VHT_BW;
		}
	else if(RX_HT == 1){		
		snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "  BW:%s", BW_table[RX_HT_BW]);
		RX_BW = RX_HT_BW;
		}
	else
		snprintf(BbDbgBuf, BT_TMP_BUF_SIZE,  "");
	...
	DCMD_Printf(BbDbgBuf);	
	snprintf(BbDbgBuf, BT_TMP_BUF_SIZE, "  RXSC:%s", RXSC_table[RXSC]);		
	DCMD_Printf(BbDbgBuf);
	*/
	///////////////////////////////////////////////////////	
