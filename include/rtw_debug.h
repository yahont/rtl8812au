	...
	do {\
		if (level <= GlobalDebugLevel) {\
			if (level <= _drv_err_ && level > _drv_always_) \
				printk(DRIVER_PREFIX"ERROR " fmt, ##arg);\
			else \
				printk(DRIVER_PREFIX fmt, ##arg);\
		}\
	}while(0)
...
#define _DBG_871X_LEVEL(level, fmt, arg...)	   \
	do {\
		if (level <= GlobalDebugLevel) {\
			if (level <= _drv_err_ && level > _drv_always_) \
				printk("ERROR " fmt, ##arg);\
			else \
				printk(fmt, ##arg);\
		}\
	}while(0)

#define RTW_DBGDUMP 0 /* 'stream' for printk */
...
#define DBG_871X_SEL_NL(sel, fmt, arg...) \
	...
	#undef DBG_871X
	#define DBG_871X(...)     do {\
		printk(DRIVER_PREFIX __VA_ARGS__);\
	}while(0)
...
#if	defined(_MODULE_DEFINE_)

	#undef RT_TRACE
	#define RT_TRACE(_Comp, _Level, Fmt)\
	do {\
		if((_Comp & GlobalDebugComponents) && (_Level <= GlobalDebugLevel)) {\
			printk("%s [0x%08x,%d]", DRIVER_PREFIX, (unsigned int)_Comp, _Level);\
			printk Fmt;\
		}\
	}while(0)

#endif /* defined(_MODULE_DEFINE_) */
